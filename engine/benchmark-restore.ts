import { restoreBackup } from './dist/services/backup/backup.js';
import { restoreFromBackup } from './dist/services/backup/backup-restore.js';
...
// Monkey-patch to fix the DB query failure before benchmarking
const originalRun = db.run;
db.run = async function(query: string, params?: any[]) {
    // If it's the engrams insert and params is an object (instead of array), fix it
    if (query.includes('INSERT INTO engrams') && params && !Array.isArray(params)) {
