#!/usr/bin/env python3
"""
Simplified ECE Core Launcher with MCP Integration

Starts the ECE Core server with optional MCP endpoints based on configuration.
MCP endpoints are available when mcp_enabled is true in config.yaml
"""
import logging
import os
import sys
...
# Add the ece-core/src to path to import modules
sys.path.insert(0, str(Path(__file__).parent / "ece-core" / "src"))
...
def create_modular_app() -> FastAPI:
    ...
    # Add generic health check for compatibility
    @ece_app.get("/health")
    async def health_root():
        return {"status": "ok", "service": "ECE Core Server", "mcp_enabled": getattr(settings, 'mcp_enabled', False) }
...
def main():
    ...
    print(f"🚀 Starting ECE Core Server on {settings.ece_host}:{settings.ece_port}")
    print(f"   - Main API: http://{settings.ece_host}:{settings.ece_port}")
    print(f"   - MCP Enabled: {getattr(settings, 'mcp_enabled', False)}")
    print(f"   - Authentication Required: {getattr(settings, 'ece_require_auth', False)}")
